...
export default {
  ...
  mounted() {
    fetch(`https://venomgram-server.herokuapp.com/post/feed/${this.username}/${this.from}/${this.to}`)
      .then(res => res.json())
      .then((data) => {
    ...
    interact(action, postId, username, method) {
      fetch(`https://venomgram-server.herokuapp.com/${action}`, {
        ...
        .then((data) => {
          ...
          fetch(`https://venomgram-server.herokuapp.com/post/feed/${this.username}/0/${this.to}`)
            .then(res => res.json())
            .then((data) => { 
              if(data === 'Authentication Failed.') {
                localStorage.clear();
